...
ISR(USART_RXC_vect)
{
	...
	if(RX_BUFFER[(RX_WRITE_POS-4)]=='C' && RX_BUFFER[(RX_WRITE_POS-3)]=='L' && RX_BUFFER[(RX_WRITE_POS-2)]=='E' && RX_BUFFER[(RX_WRITE_POS-1)]=='A' && RX_BUFFER[RX_WRITE_POS]=='R')
	{
		for(u8 i=0;i<31;i++)
			{
				RX_BUFFER[i]=0;
